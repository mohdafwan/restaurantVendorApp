...
import 'package:restaurant_vendor_app/models/tickets/raise_ticket.dart';
...
class IssueTicketController {
    ...
    if (attachedFile != null) {
      ...
      if (fileUrl != null) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('File upload failed'), backgroundColor: Colors.red),
        );
        return false;
      }
    }
  ...
  Future<bool> findTicketId(BuildContext context) async {
    ...
    final searchUrl = Uri.parse(
        'http://10.0.2.2:8000/ticket/?first_name=${ticket.firstName}&last_name=${ticket.lastName}&email=${ticket.email}&description=${ticket.description}');
    ...
    if (response.statusCode == 200) {
      ...
      if (tickets.isNotEmpty) {
        ticketId = tickets[0]['id']
            .toString(); // Change this according to your response structure
        return true;
      } else {
        // No tickets found
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
              content: Text('No ticket found after submission'),
              backgroundColor: Colors.red),
        );
        return false;
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Failed to retrieve ticket ID'),
            backgroundColor: Colors.red),
      );
      return false;
    }
